...
class Generate_HG:
    ...
    def __init__(self, sampling = False, num_Patients = 500):
        ...
        self.get_Bipartite(df_Patients,      'SUBJECT_ID', 'GENDER', 'C', 'G', 'Gender')
        self.get_Bipartite(df_Patients,      'SUBJECT_ID', 'EXPIRE_FLAG', 'C', 'E', 'Expire Flag')
    ...
    def saving_HG(self):
        nx.write_gml(self.HG, f'{self.folder_path}/HG_203D.gml')
...
def G_statistics(G):
    ...
    Gender   =  [v for v in Nodes if v[0]=='G']
    Expire   =  [v for v in Nodes if v[0]=='E']
    

    print(f'number of patients = {len(Patients)}')
    print(f'number of Genders = {len(Gender)}')
    print(f'number of Expires = {len(Expire)}')
