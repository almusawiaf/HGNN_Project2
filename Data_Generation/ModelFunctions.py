...
folder_path = '/home/almusawiaf/MyDocuments/PhD_Projects/Data/MIMIC_resources'
...
def load_patients_data(patients, num_Diseases):
    ...
    df_labs          = pd.read_csv(f'{folder_path}/LABEVENTS.csv')    # Lab test!
    df_microbio      = pd.read_csv(f'{folder_path}/MICROBIOLOGYEVENTS.csv')    # Microbiology!

    # 4. selected the data for the selected patients only...
    print('Use the patients inside the new DF....')
    new_Patients   = df_Patients[df_Patients['SUBJECT_ID'].isin(patients)]
    ...
    new_LabTest    = df_labs[df_labs['SUBJECT_ID'].isin(patients)]
    new_MicroBio   = df_microbio[df_microbio['SUBJECT_ID'].isin(patients)]
    ...
    new_LabTest.dropna(subset=['ITEMID'], inplace=True)
    new_MicroBio.dropna(subset=['SPEC_ITEMID'], inplace=True)
    ...
    return df_Patients, new_Diagnosis, new_Medication, new_Procedures, new_LabTest, new_MicroBio
...
def get_homogeneous_graphs(new_Diagnosis, new_Prescriptions, new_Procedures, new_LabTest, new_MicroBio):
    print('---------------------------------')
    ...
    visit_lab_df        = getDict2(new_LabTest, 'HADM_ID', 'ITEMID', 'V', 'L')
    visit_micro         = getDict2(new_MicroBio, 'HADM_ID', 'SPEC_ITEMID', 'V', 'B')
    ...
    VL_edges = getEdges(visit_lab_df,  'HADM_ID', 'ITEMID')  
    VB_edges = getEdges(visit_micro,  'HADM_ID', 'SPEC_ITEMID')  

    print('Done --> Getting the Homogeneous graphs...')
    
    return   CV_edges , VD_edges , VP_edges , VM_edges, VL_edges, VB_edges



def get_Heterogeneous_graph(CV_edges , VD_edges , VP_edges , VM_edges, VL_edges, VB_edges):
    print('Creating the Heterogeneous graph...')

    edge_set = CV_edges + VD_edges + VP_edges + VM_edges + VL_edges + VB_edges #+ VI_edges
    ...
    L_Nodes = [v for v in Nodes if v[0]=='L']
    B_Nodes = [v for v in Nodes if v[0]=='B']
    ...
    The_Graph = nx.Graph()
    The_Graph.add_nodes_from(C_Nodes + V_Nodes + M_Nodes + D_Nodes + P_Nodes + L_Nodes + B_Nodes)
...
def get_Nodes(G):
    ...
    print(f'number of Diagnoses = {len(D_Nodes)}')
    print(f'number of Procedures = {len(P_Nodes)}')
    print(f'number of Lab tests = {len(L_Nodes)}')
    print(f'number of MicroBiology tests = {len(B_Nodes)}')

    return C_Nodes, V_Nodes, M_Nodes, D_Nodes, P_Nodes, L_Nodes, B_Nodes, G
...
def mark_all_nodes(Nodes, C_Nodes, V_Nodes, M_Nodes, D_Nodes, P_Nodes, L_Nodes, B_Nodes):
    print('Assingning embedding to the other nodes.')
    print('\t The GNN model shall identify the other nodes as well, right?.')

    nodes_by_type = {'C':0, 'V':1, 'M': 2, 'D': 3, 'P':4, 'L': 5, 'B': 6}

    nodes_list = {'C': C_Nodes, 'V': V_Nodes, 'M': M_Nodes, 'D': D_Nodes, 'P': P_Nodes, 'L': L_Nodes, 'B': B_Nodes}
